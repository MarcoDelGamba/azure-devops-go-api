...
import (
	...
	"github.com/MarcoDelGamba/azure-devops-go-api/azuredevops/v6/notification"
	"net/http"
	"net/url"
	"strconv"
)
...
func (client *ClientImpl) CreateTestNotification(ctx context.Context, args CreateTestNotificationArgs) (*Notification, error) {
	...
	resp, err := client.Client.Send(ctx, http.MethodPost, locationId, "6.0", nil, queryParams, bytes.NewReader(body), "application/json", "application/json", nil)
	if err != nil {
		return nil, err
	}
